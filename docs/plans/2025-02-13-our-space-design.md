...
Личный сайт для пары: приватное пространство с фото-таймлайном, записками, статусами настроения и совместным плейлистом.
...
Два пользователя — создатель и приглашённый партнёр.
...
Одна запись на пару. Хранит дату начала отношений (для счётчика) и одноразовый инвайт-токен.

```prisma
model Couple {
  id          String   @id @default(cuid())
  startDate   DateTime // дата начала отношений
...
- Генерация инвайт-ссылки (одноразовой)

## Флоу авторизации

1. Первый пользователь регистрируется на сайте — создаётся `User` + `Couple` (с `user2Id = null`)
2. В настройках генерирует инвайт-ссылку (`/invite/[token]`)
3. Отправляет ссылку партнёру
4. Партнёр переходит по ссылке → форма регистрации → создаётся `User`, привязывается к `Couple` как `user2Id`
...
> **Примечание:** Визуальный дизайн сайта (цветовая палитра, типографика, анимации, компоновка элементов, адаптивность) в этом документе не зафиксирован и будет разработан отдельно. Текущий документ покрывает только архитектуру, технические решения и функционал.
